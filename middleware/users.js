...
exports.login = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({id: result._id, username: userinfo.username}, 'tecweb2223', {
            expiresIn: '1h',
        });
        res.json({token})
...
exports.getUser = async (req, res) => {
    try {
        ...
        const decoded = jwt.verify(token.replace('Bearer ', ''), 'tecweb2223');
        ...
        const db = await connect();
        const users = await db.collection('users');
        const result = await users.find({ username: currentUser }).toArray();
